...
interface DatesRangePickerProps
...
class DatesRangePicker extends RangeSelectionPicker<DatesRangePickerProps>
  ...
  protected getActiveCellsPositions(): RangeIndexes {
    ...
    */
    ...
    const fromCurrentMonthDayPositions = getDefaultEnabledDayPositions(
      allDays,
      date,
    );

    const fromPrevMonthDates = getDatesFromPrevMonth(
      date,
      allDays,
      fromCurrentMonthDayPositions[0],
    );
    const fromNextMonthDates = getDatesFromNextMonth(
      date,
      allDays,
      last(fromCurrentMonthDayPositions) + 1,
    );
  ...
  protected getSelectedRange(): string {
    ...
    return `${start ? start.format(dateFormat) : '- - -'} - ${
  ...
  protected handleChange = (
    ...
    const { start, end, localization } = this.props;
    ...
    if (isNil(start) && isNil(end)) {
      ...
      const selectedDate = buildMoment(
        this.state.date,
        firstOnPage,
        itemPosition,
        localization,
...
function getDatePosition(
  ...
  if (date.isSame(currentMonth, 'month')) {
    return (
      fromCurrentMonthDates.indexOf(date.date()) + fromPrevMonthDates.length
    );
  }
  if (date.isSame(nextMonth, 'month')) {
    ...
    if (position >= 0) {
      return (
        position + fromPrevMonthDates.length + fromCurrentMonthDates.length
      );
    }
  }
}
...
function getDatesFromNextMonth(date, allDays, nextMonthStartPosition) {
  ...
  return allDays
    .slice(nextMonthStartPosition, allDays.length)
    .map((d) => parseInt(d, 10));
}
...
function buildMoment(
  ...
  if (firstOnPage === 1 /* page starts from first day in month */) {
    ...
    result = localization
      ? moment(dateOptions).locale(localization)
      : moment(dateOptions);
  } else {
    /* page starts from day in previous month */
    const dateOptions = {
      year: pageReferenceDate.month()
        ? pageReferenceDate.year()
        : pageReferenceDate.year() - 1,
