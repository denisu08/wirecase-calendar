...
class HourPicker extends SingleSelectionPicker<HourPickerProps>
  ...
  protected buildCalendarValues(): string[] {
    ...
    */
    return range(0, 24)
      ...
      .map((hour) =>
        buildTimeStringWithSuffix(hour, '00', this.props.timeFormat),
      );
  }
  ...
  protected getDisabledPositions(): number[] {
    /*
      ...
      (position in array returned by `this.buildCalendarValues`).
    */
    const { disable, minDate, maxDate } = this.props;
    ...
    if (isArray(disable)) {
      disabledByDisable = concat(
        disabledByDisable,
        disable
          .filter((date) => date.isSame(this.state.date, 'day'))
          .map((date) => date.hour()),
      );
    }
    if (minDate) {
    ...
    const result = sortBy(
      uniq(concat(disabledByDisable, disabledByMaxDate, disabledByMinDate)),
    );
    if (result.length > 0) {
      return result;
    }
